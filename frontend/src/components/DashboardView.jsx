    ...
    <section className="flex flex-col gap-8 pb-12 font-sans text-fg">
      ...
      {activeSubTab === 'dashboard' && (
        <>
          ...
          {/* 5. INTERACTIVE CAMPAIGNS SECTION */}
          <div id="campaign-details-section" className="bg-white p-10  transition-all flex flex-col gap-6 scroll-mt-6">
            ...
            {layoutMode === 'kanban' ? (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
                {Object.entries(columns).map(([colName, colItems]) => {
                  ...
                  return (
                    <div key={colName} className="bg-slate-100 border border-slate-100 rounded-2xl p-4 min-h-[350px] flex flex-col gap-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                        ...
                        <span className="bg-white text-slate-500 text-[9px] font-bold px-2 py-0.5 rounded-full border border-slate-150 shadow-sm">
                          {colItems.length}
                        </span>
                      </div>
                            ...
                            {paginatedCompleted.map((campaign) => {
                              ...
                              return (
                                <article
                                  ...
                                  className="bg-white  rounded-2xl p-4 flex flex-col gap-3 transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5 cursor-pointer hover:border-blue-200"
                                >
                                  <div className="flex items-start justify-between gap-2">
                                    <h3 className="text-xs font-bold text-slate-900 truncate flex-1 leading-normal">{campaign.name}</h3>
                                  </div>
                                  <p className="text-[11px] text-slate-400 line-clamp-2 leading-relaxed font-medium">{campaign.subject}</p>
                                  <div className="flex flex-wrap gap-1">
                                    <span className="bg-slate-50 text-slate-500 text-[9px] font-bold px-2 py-0.5 rounded-md border border-slate-100">{total} recipients</span>
                                    <span className="bg-slate-50 text-slate-500 text-[9px] font-bold px-2 py-0.5 rounded-md border border-slate-100">{campaign.senderRotationMode}</span>
                                  </div>
                                  <div className="mt-1">
                                    ...
                                    <div className="flex justify-between text-[8px] font-bold text-slate-400 mt-1 uppercase tracking-wider">
                                      ...
                                      <span>{stats.sent} sent &middot; {stats.failed} failed</span>
                                    </div>
                                  </div>
                                  <div className="flex items-center justify-between pt-3 border-t border-slate-100 gap-2">
                                    <span className="text-[9px] font-bold text-slate-400 truncate">{formatDate(campaign.createdAt)}</span>
                                    <button
                                      onClick={(e) => { e.stopPropagation(); navigate('/compose', { state: { duplicateCampaign: campaign } }); }}
                                      className="h-7 px-3 bg-white border border-slate-200 hover:bg-slate-50 text-slate-500 hover:text-slate-800 text-[10px] font-bold rounded-xl transition-all cursor-pointer shadow-sm flex items-center justify-center gap-1.5"
                                      type="button"
                                    >
                          ...
                          colItems.map((campaign) => {
                            ...
                            return (
                              <article
                                key={campaign._id}
                                onClick={() => navigate(`/campaigns/${campaign._id}`)}
                                className="bg-white border border-slate-100 rounded-2xl p-4 flex flex-col gap-3 transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5 cursor-pointer hover:border-blue-200"
                              >
                                <div className="flex items-start justify-between gap-2">
                                  <h3 className="text-xs font-bold text-slate-900 truncate flex-1 leading-normal">{campaign.name}</h3>
                                </div>
                                  ...
                                  <div className="flex items-center gap-1.5 flex-shrink-0">
                                    ...
                                    {campaign.status === 'Draft' && (
                                      <button
                                        ...
                                        className="h-7 px-3.5 bg-slate-900 hover:bg-slate-800 text-white text-[10px] font-bold rounded-xl transition-all active:scale-95 cursor-pointer shadow-sm"
                                        type="button"
                                      >
                                        Launch
                                      </button>
